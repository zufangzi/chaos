...
func main() {
	...
	common.GetShell("mkdir -p " + SCRIPT_HOME)
	common.GetShell("wget " + *ARGS_SCRIPT_WGET_HOME + START_SCRIPT + " && mv " + START_SCRIPT + " " + SCRIPT_HOME)
	common.GetShell("wget " + *ARGS_SCRIPT_WGET_HOME + STOP_SCRIPT + " && mv " + STOP_SCRIPT + " " + SCRIPT_HOME)
	common.GetShell("wget " + *ARGS_SCRIPT_WGET_HOME + DELETE_SCRIPT + " && mv " + DELETE_SCRIPT + " " + SCRIPT_HOME)
...
func startProcessor(msg *dockerapi.APIEvents, shortId string) {
	...
	cnt, _ := strconv.Atoi(common.GetShell("docker inspect " + shortId + " | grep SERVICE_NAME | wc -l"))
	if cnt == 0 {
		return
	}
	...
	common.GetShell("sh " + SCRIPT_HOME + START_SCRIPT + " " + shortId + " " + ip)
...
func dieProcessor(msg *dockerapi.APIEvents, shortId string) {
	...
	common.GetShell("sh " + SCRIPT_HOME + STOP_SCRIPT + " " + shortId)
	log.Println("Step3: begin to process log collection")
}
...
func destroyProcessor(msg *dockerapi.APIEvents, shortId string) {
	...
	common.GetShell("sh " + SCRIPT_HOME + DELETE_SCRIPT + " " + shortId)
	delIp(shortId)
...
func getHostname(id string, simpleCut bool) string {
	if !simpleCut {
		return common.GetShell("docker inspect -f {{.Config.Hostname}} " + id)
	} else {
